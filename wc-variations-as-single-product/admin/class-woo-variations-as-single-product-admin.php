...
class Woo_Variations_As_Single_Product_Admin {
	...
	public function get_variations_as_product_settings() {
		...
		$settings = array(
			...
			'disable_category_page_on_selected_or_all'   => array(
				'type'              => 'radio',
				'id'                => 'wvasp_disable_category_page_on_selected_or_all',
				'row_class'         => 'wvasp-inner-radio',
				'options'           => array(
					'all'      => __( 'All Categories', 'wc-variations-as-single-product' ),
					'selected' => __( 'Selected Categories (Premium)', 'wc-variations-as-single-product' ),
				),
				'default'           => 'all',
				'value'             => 'all',
			...
			'hie_backorder_variation_product'            => array(
				...
				'type'              => 'checkbox',
				'desc'              => __(
					'Hide product variations with backorder status. Sometimes, it\'s necessary to hide backorder variations, and this setting will enable that. (Will work with both \'Allow, but notify customer\' and 
				\'Allow\')',
					'wc-variations-as-single-product'
				) . ' <a class="premium-tag-link" href="https://storeplugin.net/plugins/variations-as-single-product-for-woocommerce/?utm_source=activesite&utm_campaign=singlevar&utm_medium=link" target="_blank">' . __( 'Get Premium', 'wc-variations-as-single-product' ) . '</a>',
	...
	public function save_settings_ajax() {
		...
		if ( ! check_ajax_referer( 'wvasp_save_settings_nonce', 'nonce', false ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid nonce.', 'wc-variations-as-single-product' ) ) );
			return;
		}

		// Check if user has permission
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			wp_send_json_error( array( 'message' => __( 'You do not have permission to perform this action.', 'wc-variations-as-single-product' ) ) );
			return;
		}
		...
		wp_send_json_success(
			array(
				'message'               => __( 'Settings saved successfully!', 'wc-variations-as-single-product' ),
				'varible_product_count' => $variable_product_count,
			)
		);
	}
	...
	public function batch_update_product_variations() {
		...
		$variable_product_type = array_unique( array_merge( array( 'variable' ), apply_filters( 'wvasp_processing_extra_variable_types', array() ) ) ); // e.g., ['pw-gift-card']
		...
		wp_send_json_success( array( 'processed' => $product_data_count ) );
	}
